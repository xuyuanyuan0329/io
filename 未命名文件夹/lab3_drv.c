...
static enum mitm_handler_result mitm_from_slave(struct mitm *mitm, struct sk_buff *skb)
{
    ...
    //data size less 70bytes
    if (skb->len < 70) {
        printk(KERN_DEBUG "data size < 70\n");
        printk(KERN_DEBUG "---data size = %d, sIP= %pI4  dIP= %pI4 \n", skb->len, &src_ip, &dest_ip);
        memset(buff, 0, 1024);
        sprintf(buff, "data size = %d, sIP= %pI4  dIP= %pI4 \n", skb->len, &src_ip, &dest_ip);
    }
